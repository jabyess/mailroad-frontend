...
class MediaUploadForm extends React.Component {
		...
		if (rejectedFiles.length > 0) {
			window.alert(
				rejectedFiles.length +
					" file(s) not loaded, either too big or wrong type"
			)
		}
	}
	...
	handleHeightChange(height, imageIndex, sizeIndex) {
		this.setState(
			state => (state.sizeInputs[imageIndex][sizeIndex].height = height)
		)
	}

	handleWidthChange(width, imageIndex, sizeIndex) {
		this.setState(
			state => (state.sizeInputs[imageIndex][sizeIndex].width = width)
		)
	}
	...
	addSizeRow(imageIndex) {
		if (this.state.sizeInputs[imageIndex].length < 5) {
			this.setState(state =>
				state.sizeInputs[imageIndex].push({ height: "", width: "" })
			)
		}
	}
	...
	render() {
		const imageSizeInputs =
			this.state.sizeInputs && this.state.sizeInputs.length > 0 ? (
				<ImageSizeInputs
		...
		return (
			<div className="mediaUploadForm columns">
				...
				<div className="mediaUploadForm__inputs column">{imageSizeInputs}</div>
			</div>
		)
	}
}
